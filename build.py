...
# Configurações
PROJECT_NAME = "MarcatiliKawanoGUI"
...
def check_python():
    ...
    if success:
        ...
        log_error(
            "Python 3 não encontrado. Instale com: sudo apt install python3 python3-pip python3-venv"
        )
        return False
...
def check_tkinter():
    ...
    success, stdout, _ = run_command(
        "python3 -c 'import tkinter' 2>/dev/null && echo 'OK'", capture_output=True
    )
...
def install_system_dependencies():
    ...
    response = input(
        f"{Colors.YELLOW}Instalar dependências do sistema? (s/N): {Colors.RESET}"
    )
    if response.lower() == "s":
...
def install_python_dependencies():
    ...
    deps = ["numpy>=1.21.0", "matplotlib>=3.5.0", "pyinstaller>=5.0.0"]
...
def print_summary():
    ...
    print(
        f"{Colors.GREEN}{Colors.BOLD}{'COMPILAÇÃO CONCLUÍDA COM SUCESSO!':^70}{Colors.RESET}"
    )
    ...
    if executable.exists():
        ...
        print(
            f"{Colors.WHITE}📏 Tamanho: {Colors.YELLOW}{size_mb:.1f} MB{Colors.RESET}"
        )
...
def main():
    ...
    parser.add_argument(
        "--quick", action="store_true", help="Compilação rápida (pula verificações)"
    )
    parser.add_argument(
        "--debug", action="store_true", help="Compilação com modo debug"
    )
    parser.add_argument(
        "--clean", action="store_true", help="Limpar todos os arquivos gerados"
    )
    parser.add_argument(
        "--no-venv", action="store_true", help="Não usar ambiente virtual"
    )
    ...
    print("╔══════════════════════════════════════════════════════════════════╗")
    print(
        f"║     {PROJECT_NAME} - Build Script para Linux Mint v{PROJECT_VERSION}     ║"
    )
    print("╚══════════════════════════════════════════════════════════════════╝")
