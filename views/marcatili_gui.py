...
class MarcatiliKawanoGUI:
    ...
    def _create_left_panel(self, parent):
        ...
        tk.Label(
            scrollable,
            text="MARCATILI-KAWANO ANALYZER",
        ...
        # Índices das 5 regiões
        nframe = ttk.LabelFrame(
            scrollable, text="Índices de refração (5 regiões)", padding=8
        )
        ...
        ttk.Radiobutton(
            pol_frame, text="TE (E_x)", variable=self.polarization_var, value=1
        ).pack(side=tk.LEFT, padx=5)
        ttk.Radiobutton(
            pol_frame, text="TM (E_y)", variable=self.polarization_var, value=2
        ).pack(side=tk.LEFT, padx=5)
        ...
        self.calc_btn = ttk.Button(
            btn_frame, text="CALCULAR", command=self._on_calculate
        )
        self.calc_btn.pack(fill=tk.X, pady=3)
        self.clear_btn = ttk.Button(
            btn_frame, text="🗑 LIMPAR TUDO", command=self._on_clear
        )
        self.clear_btn.pack(fill=tk.X, pady=3)
        self.save_btn = ttk.Button(
            btn_frame, text="SALVAR GRÁFICOS", command=self._on_save, state="disabled"
        )
        self.save_btn.pack(fill=tk.X, pady=3)
        self.export_btn = ttk.Button(
            btn_frame, text="EXPORTAR DADOS", command=self._on_export, state="disabled"
        )
    ...
    def _create_right_panel(self, parent):
        ...
        self.figure.subplots_adjust(
            left=0.08, right=0.92, top=0.92, bottom=0.08, hspace=0.35, wspace=0.35
        )
        ...
        # Primeiro canvas (criar antes de tentar desenhar)
        ...
        # Só depois de criar o canvas podemos chamar _clear_plots
        self._clear_plots()
        return panel

    def _clear_plots(self):

        # Se canvas ainda não existe, sai sem fazer nada
    ...
    def _validate_inputs(self):
        try:
            ...
            float(self.lambda_var.get())
            float(self.n1_var.get())
            float(self.n2_var.get())
            float(self.n3_var.get())
            float(self.n4_var.get())
            float(self.n5_var.get())
            int(self.mode_x_var.get())
            int(self.mode_y_var.get())
    ...
    def _on_calculate(self):
        ...
        try:
            self.controller.set_parameters(
                ...
                n3=float(self.n3_var.get()),
                n4=float(self.n4_var.get()),
                n5=float(self.n5_var.get()),
    ...
    def _update_plots(self):
        ...
        self.ax2.set_xlabel("x (μm)")
        self.ax2.set_ylabel(f"{tipo} (u.a.)")
        self.ax2.set_title("Corte em y = 0")
        self.ax2.axvline(-a_um, color="r", linestyle="--")
        self.ax2.axvline(a_um, color="r", linestyle="--")
        ...
        self.ax3.set_xlabel("y (μm)")
        self.ax3.set_ylabel(f"{tipo} (u.a.)")
        self.ax3.set_title("Corte em x = 0")
        self.ax3.axvline(-b_um, color="b", linestyle="--")
        self.ax3.axvline(b_um, color="b", linestyle="--")
        ...
        self.figure.suptitle(
            f"Marcatili-Kawano - Modo {tipo_pol}_{{{params.mode_x}{params.mode_y}}}",
            fontsize=14,
        )
        self.canvas.draw()
    ...
    def _on_clear(self):
        if messagebox.askyesno(
            ...
            self.n1_var.set("1.5")
            self.n2_var.set("1.0")
            self.n3_var.set("1.45")
            self.n4_var.set("1.45")
            self.n5_var.set("1.45")
            self.mode_x_var.set("0")
            self.mode_y_var.set("0")
